...
#include <FastLED.h>
#include <EEPROM.h> // EEPROM bibliotheek toevoegen
...
// EEPROM adres om aantalRingen op te slaan
const int EEPROM_AANTAL_RINGEN_ADRES = 0;
...
void setup() {
    ...
    // Eenmalige reset van aantal ringen naar nul
    bool resetTriggered = true; // Verander naar true om reset eenmalig te activeren

    if (resetTriggered) {
        resetAantalRingen(); // Roep de resetfunctie aan
    } else {
        aantalRingen = EEPROM.read(EEPROM_AANTAL_RINGEN_ADRES) - 1;

        // Zorg ervoor dat het aantal ringen niet negatief wordt
        if (aantalRingen < 0) aantalRingen = 0;

        Serial.print("Aantal Ringen geladen uit EEPROM (verlaagd met 1): ");
        Serial.println(aantalRingen);
    }
}
void resetAantalRingen() {
    EEPROM.write(EEPROM_AANTAL_RINGEN_ADRES, 0); // Zet aantal ringen op 0
    aantalRingen = 0; // Zorg ervoor dat de runtime-variabele ook wordt bijgewerkt
    Serial.println("Aantal Ringen is teruggezet naar nul.");
}
...
void updateLedStrip() {
    ...
    if (fietsOmwentelingen >= doel) {
        ...
        if (!doelBereikt) {
            ...
            // Opslaan van aantalRingen in EEPROM
            EEPROM.write(EEPROM_AANTAL_RINGEN_ADRES, aantalRingen);
            Serial.print("Aantal Ringen opgeslagen in EEPROM: ");
            Serial.println(aantalRingen);
        }
    } else {
